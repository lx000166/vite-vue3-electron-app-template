...
<template>
  ...
  <div class="inner">
    ...
    <span class="title text-sm text-[#1a1a1a] w-[220px] truncate text-left">{{ title }}</span>
...
<style scoped>
.blur { position: absolute; inset: 0; z-index: 1; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); }
.blur-white { background: rgba(255,255,255,0.05); }
.bg { position: absolute; inset: 0; z-index: 2; }
.bg-white-60 { background: rgba(255,255,255,0.6); }
.bg-gradient { background: linear-gradient(to bottom, rgba(255,255,255,0.6), rgba(255,255,255,0.2)); }
