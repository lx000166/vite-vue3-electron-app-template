...
所有模块、方法、函数、功能团必须添加注释，模板中相对独立的功能区域之间也要有合理的分隔与注释。

### 文件级注释

每个源文件顶部使用 JSDoc 风格描述模块职责：
...
`.vue` 文件在 `<template>` 前使用 HTML 注释块描述组件职责、Props/Emits、布局结构：

```html
<!--
  ComponentName — 组件功能概述
 ...
 * @returns 返回值说明
 */
export function useXxx(name: string): Result { ... }
```

### 模板功能区域分隔

模板中相对独立的功能区域使用 HTML 注释分隔：
